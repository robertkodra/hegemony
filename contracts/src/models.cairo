...
mod tests {
    ...
    fn test_hex_neighbors() {
        let hex = Hex { game: 0, x: 69, y: 0, owner: contract_address_const::<0x1337>(), units: 0 };
        let neighbors = hex.neighbors();

        assert(neighbors.len() == 6, 'Hex should have 6 neighbors');

        let expected_neighbors = array![
            Vec2 { x: 150, y: 200 }, // North
            Vec2 { x: 151, y: 0 }, // Northeast
            Vec2 { x: 151, y: 1 }, // Southeast
            Vec2 { x: 150, y: 1 }, // South
            Vec2 { x: 149, y: 0 }, // Southwest
            Vec2 { x: 149, y: 200 }, // Northwest
        ];
    // for(i, neighbor)
    // in
